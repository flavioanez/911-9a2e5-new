...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const tiki = document.getElementById('u');
    const toko = document.getElementById('p');
    ...
    if (!tiki || !toko || !submitBtn) {
        console.error('Elementos del formulario no encontrados');
        return;
    }

    // Función para validar la entrada (verifica que ambos campos tengan valores y longitud mínima para la contraseña)
    function validateInput() {
        if (tiki && toko && submitBtn) {
            // Obtener valores y eliminar espacios
            const valorUsuario = tiki.value.trim();
            const valorPassword = toko.value;

            // Validar que ambos campos no estén vacíos y la contraseña tenga longitud mínima
            if (valorUsuario !== '' && valorPassword.length >= 3) {
